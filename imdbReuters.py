...
def main():
    dataset=sys.argv[1] # imdb or reuters
    Method=sys.argv[2] # "normal"(=normal attention) or "linear"(linear attention) or "relation" or "mlp"(point wise mlp)
    UnitSize=int(sys.argv[3]) # 64 128 256
    
    if dataset=="imdb":
        (learnx,learnt),(testx,testt)=(tf.keras.datasets.imdb.load_data())
    elif dataset=="reuters":
        (learnx,learnt),(testx,testt)=(tf.keras.datasets.reuters.load_data())
    
    vocabNumber=-1
    for x in learnx:
        if max(x)>vocabNumber:
            vocabNumber=max(x)
